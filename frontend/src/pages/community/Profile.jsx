  ...
  if (error) {
    return (
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-sm text-gray-600">If you don't have a profile yet, please create one from the sidebar.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <header>
        ...
        <p className="text-gray-600">Status: {p.profileStatus}</p>
      </header>
      ...
      <section className="bg-white rounded-2xl shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field label="Total Population" value={p.demographics?.totalPopulation} />
          <Field label="Total Households" value={p.demographics?.totalHouseholds} />
        </div>
      </section>
...
function Field({ label, value, full }) {
  return (
    <div className={full ? "md:col-span-2" : ""}>
      ...
      <div className="px-3 py-2 rounded-lg border bg-gray-50 text-gray-800">
        {value || "-"}
      </div>
    </div>
  );
