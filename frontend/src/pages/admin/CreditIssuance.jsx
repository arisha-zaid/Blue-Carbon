...
export default function CreditIssuance() {
  ...
  <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
    <div>
      <h1 className="text-3xl font-bold text-white">Credit Issuance</h1>
    ...
    <button
      ...
      className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400 hover:shadow-[0_0_10px_#14b8a6] transition-all duration-300"
    >
      <RefreshCcw className="w-4 h-4" /> Clear History
    </button>
  </header>
  ...
  <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <form
      onSubmit={submit}
      className="bg-[#1a1a1a] rounded-2xl shadow p-6 lg:col-span-2"
    >
      <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
        <ShieldCheck className="w-5 h-5 text-emerald-400" /> Issuance Details
      </h3>
      ...
      <div className="mt-6 flex items-center justify-end gap-3">
        <button
          type="button"
          onClick={resetForm}
          className="px-4 py-2 rounded-full border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400 hover:shadow-[0_0_8px_#14b8a6] transition-all duration-300"
        >
          Reset
        </button>
        <button
          type="submit"
          disabled={submitting || !canSubmit}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-600 text-white hover:shadow-[0_0_12px_#34d399] hover:brightness-110 disabled:opacity-60 transition-all duration-300"
        >
          <ArrowRightCircle className="w-4 h-4" />
          {submitting ? "Issuing..." : "Issue Credits"}
        </button>
    ...
    <div className="bg-[#1a1a1a] rounded-2xl shadow p-6">
      ...
      <div className="rounded-2xl border border-gray-700 p-5 space-y-4">
        <Row
      ...
      <div className="mt-4 text-xs text-gray-500">
        Certificate will be anchored on-chain with a unique TxID. You can
        copy/share the TxID or scan the QR from the history section after
        issuance.
      </div>
    </div>
  </section>
  ...
  {/* History */}
  <section className="bg-[#1a1a1a] rounded-2xl shadow p-6">
    <div className="flex items-center justify-between mb-4">
      <h3 className="font-semibold text-white">Recent Issuances</h3>
      <div className="text-sm text-gray-400">
        Total: <span className="font-semibold text-gray-200">{history.length}</span>
      </div>
    </div>
          ...
          {history.length === 0 ? (
            ...
            history.map((h) => (
              <tr key={h.id} className="border-t border-gray-800">
                <td className="py-3 px-4 font-semibold text-gray-200">{h.id}</td>
                <td className="py-3 px-4">{h.projectId}</td>
                <td className="py-3 px-4">{h.issueTo}</td>
                <td className="py-3 px-4">{h.credits.toLocaleString()}</td>
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2 justify-end">
                    <button
                      onClick={() => handleCopy(h.txId, h.id)}
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400 hover:shadow-[0_0_8px_#14b8a6] text-sm transition-all duration-300"
                    >
                      {copied === h.id ? (
                        <>
                          <Check className="w-4 h-4" /> Copied
