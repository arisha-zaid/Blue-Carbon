...
export default function ProjectApproval() {
  ...
  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    <div>
      <h1 className="text-3xl font-bold text-white">Project Approval</h1>
  ...
  {/* Toolbar */}
  <div className="bg-[#1a1a1a] rounded-2xl shadow p-4 flex flex-col md:flex-row md:items-center gap-3 mb-6 border border-gray-800">
    <div className="relative flex-1">
      ...
      <input
        ...
        onChange={(e) => setQuery(e.target.value)}
        placeholder="Search by id, name, owner, or location"
        className="w-full pl-9 pr-3 py-2 rounded-lg bg-[#121110] text-gray-200 border border-gray-700 focus:ring-2 focus:ring-teal-500"
      />
    </div>
    ...
    <div className="flex items-center gap-2">
      ...
      <select
        value={status}
        onChange={(e) => setStatus(e.target.value)}
        className="py-2 px-3 rounded-lg bg-[#121110] text-gray-200 border border-gray-700 focus:ring-2 focus:ring-teal-500"
      >
        {STATUSES.map((s) => (
    ...
    <div className="flex items-center gap-2">
      ...
      >
        {TYPES.map((t) => (
          <option key={t} value={t} className="bg-[#1a1a1a] text-gray-200">
            {t}
          </option>
        ))}
      </select>
  ...
  <div className="bg-[#1a1a1a] rounded-2xl shadow overflow-hidden border border-gray-800">
    <table className="w-full text-left">
      <thead className="bg-[#121110] text-gray-400 text-sm border-b border-gray-800">
        <tr>
          <th className="py-3 px-4">
            <input
              type="checkbox"
      ...
      <tbody>
        {filtered.length === 0 ? (
          ...
          filtered.map((r) => (
            <tr key={r.id} className="border-t border-gray-800 hover:bg-[#121110]/60">
              <td className="py-3 px-4">
                <input
                  type="checkbox"
                  checked={selectedIds.includes(r.id)}
              ...
              <td className="py-3 px-4 text-gray-300">{r.owner}</td>
              <td className="py-3 px-4 text-gray-300">{r.type}</td>
              <td className="py-3 px-4 text-gray-300">{r.location}</td>
              <td className="py-3 px-4 text-gray-300">{r.predictedCO2}</td>
              <td className="py-3 px-4 text-gray-300">{r.submittedAt}</td>
              <td className="py-3 px-4">
                <span
                  className={`inline-flex items-center justify-center whitespace-nowrap px-3 py-1 rounded-full text-xs ${
      r.status === "MRV Complete"
        ...
        : "bg-gray-800 text-gray-400"
              ...
              <td className="py-3 px-4">
                <div className="flex items-center gap-2 justify-end">
                  <button
                    onClick={() => openDrawer(r)}
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400 text-sm"
                    title="View Details"
                  >
                    <Eye className="w-4 h-4" /> View
                  </button>
                  {r.status === "MRV Complete" ? (
                    <>
                      <button
                        onClick={() => openAction("approve")}
                        className="inline-flex items-center gap-2 px-2 py-1 rounded-lg border border-emerald-600  text-emerald-500 border border-gray-600 hover:shadow-[0_0_10px_#34d399]"
                      >
                        <CheckCircle2 className="w-4 h-4" /> Approve
                      </button>
                      <button
                        onClick={() => openAction("reject", "Need clearer MRV evidence.")}
                        className="inline-flex items-center gap-2 px-2 py-1 rounded-lg text-rose-500 border border-gray-600 hover:shadow-[0_0_10px_#f43f5e]"
                      >
  ...
  {/* Drawer */}
  {drawer ? (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[520px] bg-[#1a1a1a] shadow-2xl z-40 flex flex-col border-l border-gray-800">
      <div className="p-4 border-b border-gray-800 flex items-center justify-between">
        <div className="font-semibold text-gray-200">Project Details</div>
        <button
          onClick={closeDrawer}
          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400"
        >
          <ChevronRight className="w-4 h-4 rotate-180" /> Close
        </button>
      </div>
      ...
      <div className="p-5 space-y-5 overflow-auto">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-2xl font-bold text-white">{drawer.name}</div>
            <div className="text-xs text-gray-500">{drawer.id}</div>
          </div>
          <span
            className={`px-2 py-1 rounded-full text-xs ${
        ...
        <div>
          ...
          <p className="mt-1 text-gray-200">{drawer.description}</p>
        </div>
        ...
        <div className="pt-2 flex items-center gap-3">
          <button
            onClick={() => openAction("approve")}
            className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-emerald-600  text-emerald-500 border border-emerald-600 hover:shadow-[0_0_10px_#34d399]"
          >
            <CheckCircle2 className="w-4 h-4" /> Approve
          </button>
          <button
            onClick={() => openAction("reject")}
            className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-rose-600  text-rose-500 border border-rose-600 hover:shadow-[0_0_10px_#f43f5e]"
          >
            <XCircle className="w-4 h-4" /> Request Changes
          </button>
          <button
            onClick={closeDrawer}
            className="ml-auto inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400"
          >
            <ChevronLeft className="w-4 h-4" /> Back
          </button>
        </div>
  ...
  {action.open ? (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-[#1a1a1a] rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-800">
          ...
          {action.mode === "reject" ? (
            <>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Provide clear, actionable feedback"
                className="w-full px-3 py-2 rounded-lg bg-[#121110] text-gray-200 border border-gray-700 focus:ring-2 focus:ring-teal-500"
              />
            </>
          ) : (
        ...
        <div className="p-4 border-t border-gray-800 flex items-center justify-end gap-3">
          <button
            onClick={closeAction}
            className="px-4 py-2 rounded-lg border border-gray-700 text-gray-300 hover:border-teal-500 hover:text-teal-400"
          >
            Cancel
          </button>
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg  ${
              action.mode === "approve" ? "text-emerald-700 border border-emerald-700 hover:shadow-[0px_0px_10px_emerald-600]" : 
              "bg-rose-600"
            } hover:brightness-110`}
          >
            {action.mode === "approve" ? (
