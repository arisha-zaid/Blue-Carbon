...
const updateSchema = new mongoose.Schema({
  status: {
    type: String,
    enum: ['submitted', 'under_review', 'investigating', 'resolved', 'rejected', 'escalated'],
    required: true
  },
  message: {
    type: String,
...
complaintSchema.pre('save', async function(next) {
  if (this.isNew) {
    ...
    this.complaintNumber = `CMP${year}${month}${String(sequence).padStart(4, '0')}`;
  }
  next();
});
...
// Methods
complaintSchema.methods.addUpdate = function(status, message, updatedBy, isPublic = true) {
  this.updates.push({
    status,
    message,
    updatedBy,
...
complaintSchema.methods.resolve = function(resolutionData, resolvedBy) {
  ...
  return this.addUpdate('resolved', 
...
complaintSchema.statics.getStatistics = async function() {
  ...
  return {
    ...
    avgResolutionDays: avgResolutionTime[0] ? 
      Math.round(avgResolutionTime[0].avgTime / (1000 * 60 * 60 * 24)) : 0,
