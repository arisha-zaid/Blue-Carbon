...
const projectSchema = new mongoose.Schema({
  ...
  team: [{
    ...
    role: {
      type: String,
      enum: ['project_manager', 'technical_lead', 'environmental_scientist', 
             'community_coordinator', 'financial_manager', 'advisor']
    },
    permissions: [{
      type: String,
  ...
  status: {
    type: String,
    enum: ['draft', 'proposed', 'under_review', 'approved', 'active', 
           'paused', 'completed', 'verified', 'rejected', 'cancelled'],
    default: 'draft'
  },
  phase: {
    type: String,
    enum: ['planning', 'implementation', 'monitoring', 'verification', 'maintenance']
  },
  startDate: Date,
  endDate: Date,
  ...
  documents: [{
    ...
    type: {
      type: String,
      enum: ['proposal', 'report', 'certificate', 'image', 'video', 'other']
    },
    ipfsHash: String,
    url: String,
    uploadedBy: {
  ...
  risks: [{
    category: {
      type: String,
      enum: ['environmental', 'financial', 'technical', 'social', 'regulatory']
    },
    description: String,
    probability: {
      type: String,
...
projectSchema.virtual('fundingPercentage').get(function() {
  return this.funding.goal > 0 ? 
    Math.round((this.funding.raised / this.funding.goal) * 100) : 0;
});

projectSchema.virtual('totalCarbonReduction').get(function() {
  if (!this.carbonImpact.actualImpact.length) return 0;
  return this.carbonImpact.actualImpact.reduce((total, impact) => 
    total + impact.actualReduction, 0);
});
...
projectSchema.pre('save', function(next) {
  ...
  if (this.milestones.length > 0) {
    ...
    this.progress.overall = Math.round((completedMilestones.length / this.milestones.length) * 100);
...
projectSchema.methods.completeMilestone = function(milestoneId, evidenceData, verifiedBy) {
...
projectSchema.methods.addProgressUpdate = function(message, progress, updatedBy) {
...
projectSchema.methods.addCarbonMeasurement = function(year, month, actualReduction, measurementData) {
...
projectSchema.methods.canBeViewedBy = function(user) {
  ...
  const teamMember = this.team.find(member => 
    member.user && member.user.equals(user._id));
...
projectSchema.statics.findNearby = function(latitude, longitude, maxDistance = 1000) {
  return this.find({
