...
describe('API Integration Tests', () => {
  ...
  afterAll(async () => {
    // Cleanup test data
    if (testUserId) {
      await mongoose.connection.db.collection('users').deleteOne({ _id: testUserId });
    }
    if (testProjectId) {
      await mongoose.connection.db.collection('projects').deleteOne({ _id: testProjectId });
    }
  ...
  describe('Authentication Endpoints', () => {
    ...
    test('GET /api/auth/profile should return user data with valid token', async () => {
      if (authToken) {
        const response = await request(app)
          .get('/api/auth/profile')
  ...
  describe('Projects Endpoints', () => {
    ...
    test('GET /api/projects should handle filtering', async () => {
      const response = await request(app)
        .get('/api/projects?type=mangroves&status=active');
  ...
  describe('Location Services', () => {
    test('GET /api/locations should return location suggestions', async () => {
      const response = await request(app)
        .get('/api/locations?type=mangroves');
  ...
  describe('Data Persistence', () => {
    test('should persist user data correctly', async () => {
      if (testUserId) {
        ...
        const user = await mongoose.connection.db.collection('users').findOne({ _id: new mongoose.Types.ObjectId(testUserId) });
